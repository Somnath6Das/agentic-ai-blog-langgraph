...
groq_llm = ChatGroq(model="openai/gpt-oss-120b", api_key=os.environ.get("GROQ_API_KEY")) # type: ignore
...
MAX_QUERIES = 3 # 3 search result will run
MAX_RESULTS_PER_QUERY = 4 # result per search
MAX_SNIPPET_CHARS = 400 # chars per single result
MAX_TOTAL_CHARS = 4500  # total chars across all results
MAX_EVIDENCE_ITEMS = 20 # items in the final evidence list, after dedup (after removing duplicate)

def _tavily_search(query, max_results=MAX_RESULTS_PER_QUERY):
...
def _truncate_to_budget(raw_results: List[dict], max_total_chars: int) -> List[dict]:
    out: List[dict] = []
    used = 0
    for b in raw_results:
        s = json.dumps(b, ensure_ascii=False)
        if used + len(s) > max_total_chars and out:
            break
        out.append(b)
        used += len(s)
    return out
...
def research_node(state: State) -> dict:
    ...
    blobs = _truncate_to_budget(raw_results, max_total_chars=MAX_TOTAL_CHARS)
...
#     result = run("write a blog about AI")
#     print(result["final"])
